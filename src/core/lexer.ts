...
import type { LexToken } from '@/core/tokens'

import { defaultLanguageId, getLanguageConfig } from '@/core/languages'

function classifyPunctuationToken(value: string): string {
  if (value === '(' || value === ')') {
...
function isTypeLike(type: string | null): boolean {
  return type === 'type' || type === 'primitive-type' || type === 'parameter-type'
}

function tokenizeCode(languageId: LanguageId = defaultLanguageId, code = ''): LexToken[] {
  const language = getLanguageConfig(languageId)
  const tokenPattern = new RegExp(language.tokenPattern.source, language.tokenPattern.flags)
  ...
  while ((match = tokenPattern.exec(code)) !== null) {
    ...
    if (/^\s+$/.test(value)) {
      ...
      if (value === '=') {
        ...
        if (value === '(' && previousType === 'function-call') {
          ...
          if (nextSignificantMatch?.[1] === ')') {
            type = 'punctuation-empty-call-paren'
          }
        }

        if (value === ')' && previousSignificant === '(' && previousType === 'function-call') {
          type = 'punctuation-empty-call-paren'
        }
      }
    ...
    if (type !== 'plain') {
      ...
      if (!type.startsWith('punctuation') && type !== 'operator' && type !== 'comment') {
        previousType = type
      }
    }
  }
