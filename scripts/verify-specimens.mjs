...
function extractSvgCodeRuns(svgContent) {
  const monoTextMatch =
  ...
  const inheritedFill = normalizeHex(inheritedFillMatch ? inheritedFillMatch[1] : null);
  ...
  while ((match = runPattern.exec(monoTextBlock)) !== null) {
    ...
    const color = normalizeHex(fillMatch ? fillMatch[1] : inheritedFill);
...
function buildExpectedCharStreamFromSvg(svgContent) {
...
function buildActualSegments(languageId, theme, code) {
  const segments = [];
...
function buildExpectedCodeAndColors(expectedChars) {
  const chars = [];
  const colors = [];
...
function compareSegmentsToSvg(expectedCode, expectedColors, actualSegments, maxExamples = 12) {
  const mismatches = [];
  let cursor = 0;
...
function validateCoverage() {
  ...
  const missingFiles = mappingValues.filter(
    (fileName) => !svgFiles.includes(fileName),
  );
  const unmappedFiles = svgFiles.filter(
    (fileName) => !mappingValues.includes(fileName),
  );
...
function main() {
  ...
  if (!coverage.ok) {
    ...
    if (coverage.missingThemeMappings.length > 0) {
      console.log(
        `  - missing theme mappings: ${coverage.missingThemeMappings.join(", ")}`,
      );
    }
  ...
  for (const theme of themes) {
    ...
    const { code: expectedCode, colors: expectedColors } =
      buildExpectedCodeAndColors(expectedChars);
    const actualSegments = buildActualSegments(languageId, theme, sampleCode);
    const comparison = compareSegmentsToSvg(
      expectedCode,
      expectedColors,
      actualSegments,
    );
